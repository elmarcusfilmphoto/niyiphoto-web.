...
#
# Formato de salida: WebP (mismo detalle que JPG a un peso mucho menor).
# El nombre de archivo se sanitiza (minusculas, espacios/parentesis -> guiones)
# para que sea seguro de usar en URLs.
#
# Ya optimizadas, referencia las fotos en el HTML como:
#   assets/img/gallery/<categoria>/<nombre-saneado>.webp
#
...
for cat in "${CATEGORIES[@]}"; do
  ...
  for src in "$src_dir"/*.jpg "$src_dir"/*.jpeg "$src_dir"/*.png; do
    ...
    echo "Optimizado: $cat/$base -> $cat/${safe_name}.webp"
    total=$((total+1))
  done
